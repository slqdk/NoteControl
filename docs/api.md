...
| PUT | `/note?path=` | vault:editor | Update a note. Body fields are all optional: any combination of `body`, `tags`, `locked`, `font`, `fontSize`, `width`, `versionMajor`, `versionMinor`, `state` can be sent. Omitted fields are left alone — in particular, omitting `body` preserves the on-disk body byte-for-byte and the server only rewrites the frontmatter. This is the path the Properties panel uses for property-only edits (Tags, version/state, appearance) so that a stale body snapshot in the panel cannot clobber newer content from the editor. The editor's own save sends `body` + `etag`; `etag` mismatch returns 412. Version/state are validated server-side and a bad change returns **400**: the version is monotonic (a `major.minor` below the current value is rejected; equal is allowed), `state` must be `development` or `released`, `released` requires version ≥ 1.0, and no lifecycle state is accepted at version 0.0. Entering `released` freezes a per-version archive to `.notesapp/releases/<encoded>/v{maj}.{min}.md` (see [notes.md § Release archive](notes.md#release-archive)); leaving `released` does not delete archives. **Stepper-on-released auto-bump**: if the request bumps `versionMajor` or `versionMinor` on a note that is currently `released` and doesn't itself set `state`, the server writes `state: development` in the same response — this is how the UI unlocks a released note for amendment without a separate write. The `locked` frontmatter key is a legacy flag: the server accepts it and writes it through, but the UI's read-only behaviour is driven by `state` (not `locked`), so toggling `locked` over the API has no effect on the editor surface; the server itself does not refuse writes to a `locked: true` note. |
| DELETE | `/note?path=` | vault:editor | Move note to `.notesapp/trash/`. The note's per-version release archive folder under `.notesapp/releases/<encoded>/` is **purged outright** (not moved to trash, not restorable). |
| PUT | `/note/move` | vault:editor | Move/rename a note. Old + new paths in body. The per-version release archive folder is renamed alongside the note. |
| GET | `/note/releases?path=` | vault:editor | List the note's archived released versions, newest first. Each entry carries `versionMajor`, `versionMinor`, `savedAt`. Empty list means the note has never been released. Drives the Properties panel's "Previous releases" list. Editor role (matches the rest of the note surface). |
| GET | `/note/releases/content?path=&versionMajor=&versionMinor=` | vault:editor | Read one archived release's body + frontmatter + saved-at timestamp. 404 when the requested `(major, minor)` pair has no archive. Powers the read-only archive viewer. |
| DELETE | `/note/releases?path=&versionMajor=&versionMinor=` | vault:editor | Permanently delete one archived release. Removes `v{maj}.{min}.md` from the note's release folder and sweeps the folder if it's now empty. The live note is untouched. 404 when the version has no archive. No trash, no undo — irreversible by design (the panel confirms with `window.confirm` before calling). |
| GET | `/note/release?path=` | vault:editor | **Legacy stub.** Always returns `{ exists: false, ... }` since the single-slot frozen-release model was replaced by the per-version archive above. Kept so a pre-archive frontend doesn't error; will be removed once no clients hit it. |
| GET | `/note/history?path=` | vault:editor | **Legacy stub.** Always returns `{ count: 0, latest: null }` since the 10-snapshot revert ring is gone. Kept so a pre-archive frontend's "Revert" button renders permanently disabled instead of throwing. Will be removed alongside `/note/release`. |
