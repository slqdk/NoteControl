...
│   │   ├── releases/           ← per-note frozen v{maj}.{min}.md archives
...
| `.notesapp/releases/` | Per-note **release archive**, one subfolder per note (path-encoded with `/` → `__`). Each subfolder holds one `v{major}.{minor}.md` file per past Released entry of that note (frozen at the moment of release; body + frontmatter both stored). Files are **immutable** once written; re-releasing the same `(major, minor)` overwrites in place, moving to a new version writes alongside. The Properties panel's "Previous releases" list enumerates them newest-first and offers per-version delete (server endpoint: `DELETE /note/releases`, see [api.md § Notes](api.md#notes)). On rename/move the per-note folder moves with the note; on note delete the folder is purged outright (archives don't follow a note into trash). The legacy `released.md` / `development.md` dual-slot model that previously lived here is gone — see [notes.md § Release archive](notes.md#release-archive) for the new model. |
