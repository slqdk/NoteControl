...
export interface ToggleRailButtonsProps {
   ...
   * VaultLayout no longer needs to pass them.
   *
   * Default is "toggles" for backwards compatibility with any
   * caller that doesn't pass slot.
   */
...
export function ToggleRailButtons({
  ...
  const { settings, setAppWidth, setGradientId, setTheme } = useAppearance();
      ...
      <div ref={pickerRef} className="nc-variant-picker">
        ...
        {pickerOpen && (
          <div className="nc-settings-popover" role="menu">
            {/*
              Ship 80: 3-column layout.
                Column 1 (Tree)              — tree font, tree font size, click-to-expand
                Column 2 (Notes)             — note width, note font, note font size
                Column 3 (Layout & theme)    — app width, theme, background

              Pre-Ship-80 the popover was a single tall column that
              triggered a vertical scrollbar on most laptop heights.
              Reorganising into columns keeps everything visible
              within ~700px height.

              The Win7/Win11 tree-style picker is gone. Compact is
              now the only style. The variant prop / onVariantChange
              callback are gone from this component's interface too.
            */}
            <div className="nc-settings-cols">
              {/* ============================================================
                  Column 1: Tree
                  ============================================================ */}
              <div className="nc-settings-col">
                <div className="nc-settings-col-heading">Tree</div>
              ...
              {/* ============================================================
                  ...
                  ============================================================ */}
              <div className="nc-settings-col">
                <div className="nc-settings-col-heading">Notes</div>
              ...
              {/* ============================================================
                  Column 3: Layout & theme
                  ============================================================ */}
              <div className="nc-settings-col">
                <div className="nc-settings-col-heading">Layout &amp; theme</div>
                ...
                {/*
                  Ship 80: theme picker. 'Auto' = honour OS via
                  prefers-color-scheme; 'Light' / 'Dark' force a
                  fixed scheme via [data-theme] override on <html>.
                */}
                <div className="nc-settings-section">
                  <div className="nc-variant-heading">Theme</div>
                  <button
                    type="button"
                    className={`nc-variant-option ${
                      settings.theme === 'auto' ? 'nc-variant-option-active' : ''
                    }`}
                    onClick={() => setTheme('auto')}
                    role="menuitemradio"
                    aria-checked={settings.theme === 'auto'}
                    title="Follow the operating system's light/dark setting"
                  >
                    Auto
                  </button>
                  <button
                    type="button"
                    className={`nc-variant-option ${
                      settings.theme === 'light' ? 'nc-variant-option-active' : ''
                    }`}
                  ...
                  <button
                    type="button"
                    className={`nc-variant-option ${
                      settings.theme === 'dark' ? 'nc-variant-option-active' : ''
                    }`}
            ...
            {/*
              Ship 80: Reset row spans the full popover width below
              the columns. Confirm dialog before wiping — destructive
              and undo isn't possible since values are gone from
              localStorage. Reload after wipe so all consumers
              re-initialise from defaults.
            */}
            <div className="nc-settings-divider" />
            <div className="nc-settings-section nc-settings-reset-row">
              <button
                type="button"
                className="nc-settings-reset-btn"
                onClick={() => {
                  if (
                    window.confirm(
                      'Reset all settings (theme, fonts, widths, layout, background) to defaults? This will reload the page.',
                    )
                  ) {
                    resetAllSettingsAndReload();
                  }
                }}
                title="Wipe all local settings and reload"
              >
